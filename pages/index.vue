<template>
  ...
  <div v-if="!store.isLoading" class="content d-flex flex-column align-start ga-3">
    ...
    <v-btn @click="router.replace('/movies')" class="explore-btn d-none d-sm-flex cursor-pointer transition rounded-lg"
      text="explore movies" append-icon="mdi-arrow-right" size="x-large" />
    <v-btn @click="router.replace('/movies')"
      class="bg-deep-orange d-flex d-sm-none cursor-pointer transition rounded-lg" text="explore movies"
  ...
  <div v-if="!store.isLoading" class="mt-15">
    <swiper :slidesPerView="3" :spaceBetween="30" :freeMode="true"
      :modules="[SwiperFreeMode, SwiperAutoplay, SwiperNavigation]" :grab-cursor="true" :navigation="{
      ...
      <SwiperSlide class="swiper-slide" v-for="item of store.data?.results">
        <v-img class="rounded-xl" :src="`https://image.tmdb.org/t/p/w300${item.poster_path}`" height="450" cover />
        <div class="content d-flex flex-column ga-2 align-start pa-3">
          <h3 class="text-h5">{{ item.original_title }}</h3>
          <p class="text-subtitle-1">{{ item.release_date }}</p>
        </div>
      </SwiperSlide>
    </swiper>
  </div>
