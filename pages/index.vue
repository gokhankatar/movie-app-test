<template>
    ...
    <v-row class="mt-5 d-flex flex-wrap align-center">
        <v-col v-for="item of data?.results" :key="item" cols="12" sm="6" md="4">
            <v-card class="card rounded-lg cursor-pointer" height="400">
                <v-img :src="baseImageUrl + item.backdrop_path" />
                <span class="vote pa-2 rounded-xl bg-grey-darken-1 text-white">{{ item.vote_average.toFixed(1) }}</span>
...
const url = "https://api.themoviedb.org/3/movie/now_playing?api_key=4e012d6a950f5501f23ee3e7f1e548d4&language=en-US&page=2";
